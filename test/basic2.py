...
basic_estimator = Estimator(
                      ...
                      #/train code/#      
                      code_dir="./train.py",                          # 训练代码目录,s3路径?
                      boot_file="train.py",                                        # 训练启动脚本目录,s3路径？
                      #/output/           
                      output_path="s3://ai-team/wjm_test_deploy/",                      # 训练输出位置，s3路径？        
                      #/resource allocation/
                      train_instance_type=500150,                                     # 训练环境规格 resource_id
                      train_instance_count=1,                                       # 训练环境规格数量
                      volume_size=1)                                                 # 训练附加卷

basic_estimator.fit("s3://ai-team/demo/tf_demo/input/data/", wait = True, logs = True, job_name=job_name)
...
#                     model_framework_type  = 'BASIC_FRAMEWORK',      # 模型框架
					 model_code_dir      = "./predict.py", # 推理代码目录
...
predictor_response = basic_estimator.deploy_predictor(
                        ...
                        service_models = [{"weight":100,"resourceId":500005,"instanceCount":1}])
