...
basic_estimator = Estimator(
                      ...
                      volume_size=1)                                                 # 训练附加卷
#basic_estimator.fit(["s3://aiteam/mnist/data/","s3://fdafs"], wait = True, logs = True, job_name=job_name)

model_instance=basic_estimator.create_model(
                     model_name      = "basic-modelxxx",                    # 模型名称
                     model_version   = "1.1.2",                       # 模型版本
#                     model_framework  = 500150,                      # 模型框架id
#                     model_framework_type  = 'BASIC_FRAMEWORK',      # 模型框架
					 model_code_dir      = "s3://ai-team/0611-113341/",
